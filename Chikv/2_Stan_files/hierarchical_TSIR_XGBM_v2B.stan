functions {
	...
	matrix cov_mat_est(int A,
		...
		y = (sigma_mu^2) * ones_matrix + ( sigma_x^2) * identity_mat;
...
parameters {
	...
	real<lower = 0> sigma_x;  // Volatility of each island
...
transformed parameters {
	...
	cov_mat = cov_mat_est(K, sigma_mu, sigma_x);

	// Non-centered parameterisation:
	// https://occasionaldivergences.com/posts/non-centered/
	for(t in 1:W) delta_x_mat[t,] = x_noise_mat[t,] * cov_mat; //to_row_vector( cholesky_decompose(cov_mat) * x_noise_mat[t,]');
...
model {
	...
	if (prior_dist_sigma_mu == 1)           sigma_mu ~ normal(prior_mean_sigma_mu, prior_scale_sigma_mu);                       // Half-Normal
	else if (prior_dist_sigma_mu == 2)      sigma_mu ~ cauchy(prior_mean_sigma_mu, prior_scale_sigma_mu);                       // Half-Cauchy
	else if (prior_dist_sigma_mu == 3)      sigma_mu ~ student_t(prior_df_sigma_mu, prior_mean_sigma_mu, prior_scale_sigma_mu); // Half Student-t
	else if (prior_dist_sigma_mu == 4)      sigma_mu ~ gamma(prior_shape_sigma_mu, prior_rate_sigma_mu);                        // Gamma
	else if (prior_dist_sigma_mu == 5)      sigma_mu ~ exponential(prior_rate_sigma_mu);                                        // Exponential

	if (prior_dist_sigma_x == 1)            sigma_x ~ normal(prior_mean_sigma_x, prior_scale_sigma_x);                      // Half-Normal
	else if (prior_dist_sigma_x == 2)       sigma_x ~ cauchy(prior_mean_sigma_x, prior_scale_sigma_x);                      // Half-Cauchy
	else if (prior_dist_sigma_x == 3)       sigma_x ~ student_t(prior_df_sigma_x, prior_mean_sigma_x, prior_scale_sigma_x); // Half Student-t
	else if (prior_dist_sigma_x == 4)       sigma_x ~ gamma(prior_shape_sigma_x, prior_rate_sigma_x);                       // Gamma
	else if (prior_dist_sigma_x == 5)       sigma_x ~ exponential(prior_rate_sigma_x);                                      // Exponential
