...
data {
	...
	*/
	int<lower = 1, upper = 5> prior_dist_sigma_common;
	real<lower=0> prior_mean_sigma_common;
	real<lower=0> prior_scale_sigma_common;
	real<lower=0> prior_df_sigma_common;
	real<lower=0> prior_shape_sigma_common;
	real<lower=0> prior_rate_sigma_common;
...
parameters {
  matrix[W, K] x_gp_raw;
  vector[W] x_gp_common;
  real<lower=0> sigma_x;
  real<lower=0> length_scale_x;
  real<lower=0> sigma_common;
  real<lower=0> length_scale_common;  
  real bW[C*K];
  real<lower = 0> phi;
}
...
transformed parameters {
  ...
  K_common = add_diag(cov_exp_quad(timepoints, sigma_common, length_scale_common), 1e-6);
  K_se     = add_diag(cov_exp_quad(timepoints, sigma_x, length_scale_x), 1e-6);

  L_common = cholesky_decompose(K_common);
  L_se     = cholesky_decompose(K_se);

  for(k in 1:K) {
    x_trajectory[,k] = L_se * x_gp_raw[,k] + L_common * x_gp_common;
  }
 ...
 model { 
	...
	target  += normal_lpdf(x_gp_common | 0, 1);
  
	if (prior_dist_sigma_common == 1)       target  += normal_lpdf(sigma_common | prior_mean_sigma_common, prior_scale_sigma_common);                    // Half-Normal
	else if (prior_dist_sigma_common == 2)  target  += cauchy_lpdf(sigma_common | prior_mean_sigma_common, prior_scale_sigma_common);                    // Half-Cauchy
	else if (prior_dist_sigma_common == 3)  target  += student_t_lpdf(sigma_common | prior_df_sigma_common, prior_mean_sigma_common, prior_scale_sigma_common); // Half Student-t
	else if (prior_dist_sigma_common == 4)  target  += gamma_lpdf(sigma_common | prior_shape_sigma_common, prior_rate_sigma_common);                     // Gamma
	else if (prior_dist_sigma_common == 5)  target  += exponential_lpdf(sigma_common | prior_rate_sigma_common);                                  // Exponential

	if (prior_dist_sigma == 1)       target  += normal_lpdf(sigma_x | prior_mean_sigma, prior_scale_sigma);                    // Half-Normal
	else if (prior_dist_sigma == 2)  target  += cauchy_lpdf(sigma_x | prior_mean_sigma, prior_scale_sigma);                    // Half-Cauchy
	else if (prior_dist_sigma == 3)  target  += student_t_lpdf(sigma_x | prior_df_sigma, prior_mean_sigma, prior_scale_sigma); // Half Student-t
	else if (prior_dist_sigma == 4)  target  += gamma_lpdf(sigma_x | prior_shape_sigma, prior_rate_sigma);                     // Gamma
	else if (prior_dist_sigma == 5)  target  += exponential_lpdf(sigma_x | prior_rate_sigma);                                  // Exponential
  
  
	target  += normal_lpdf(length_scale_x | 0, 1);
	target  += normal_lpdf(length_scale_common | 0, 1);
