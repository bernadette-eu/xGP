...
store_model_out_iGP  <- paste0(main_path, "4_Stan_Outputs//", "CHIKV_FP_2015_iGP",  ".RData")
store_model_out_xGP  <- paste0(main_path, "4_Stan_Outputs//", "CHIKV_FP_2015_xGP",  ".RData")
store_model_out_mxGP <- paste0(main_path, "4_Stan_Outputs//", "CHIKV_FP_2015_mxGP", ".RData")
...
sampler_init <- function(){
  list(x_gp_raw     = fit_optim$par[grepl("x_gp_raw[",     names(fit_optim$par), fixed = TRUE)],
       ...
       phi          = fit_optim$par[names(fit_optim$par) %in% "phi"]
  )
}
...
sampler_init <- function(){
  list(x_gp_raw     = fit_optim$par[grepl("x_gp_raw[",     names(fit_optim$par), fixed = TRUE)],
       x_gp_common  = fit_optim$par[grepl("x_gp_common[",  names(fit_optim$par), fixed = TRUE)],
       sigma_x      = fit_optim$par[grepl("sigma_x",       names(fit_optim$par), fixed = TRUE)],
       length_scale_x = fit_optim$par[grepl("length_scale_x",names(fit_optim$par), fixed = TRUE)],
       bW           = fit_optim$par[grepl("bW[",           names(fit_optim$par), fixed = TRUE)],
       sigma_common        = fit_optim$par[names(fit_optim$par) %in% "sigma_common"],
       length_scale_common = fit_optim$par[names(fit_optim$par) %in% "length_scale_common"],
       phi          = fit_optim$par[names(fit_optim$par) %in% "phi"]
  )
}
