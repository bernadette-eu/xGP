...
stan_file_path  <- paste0(main_path, "2_Stan_files//",   "hierarchical_TSIR_MXGBM_v2_chol", ".stan")
store_model_out <- paste0(main_path, "4_Stan_Outputs//", "CHIKV_FP_2014_MXGBM_v3_chol", ".RData")
...
###########################################################################
#
# Region: French Polynesia, CHIKV disease -----------------
#
###########################################################################

## Select ZIKV in the French polynesian islands:
key_CHIKV_filtered <- key %>% filter(REGION_ID == 0)
...
main_dataset_CHIKV <- 
  ...
  filter(REGION_ID == 0,
         VIRUS == "CHIKV",
         DATE2 <= "2015-06-21" & DATE2 >= "2014-09-01"
  ) %>% 
...
main_dataset_CHIKV <- 
  ...
  filter(REGION_ID == 0,
         VIRUS == "CHIKV",
         ISLAND %nin% "AUSTRALES",
         DATE2 >= "2014-11-02" & DATE2 <= "2015-02-2"
  ) %>% 
  arrange(ISLAND_ID, DATE2)

key_CHIKV_filtered <-
  key_CHIKV_filtered %>% 
  filter(ISLAND %nin% "AUSTRALES")
...
for (i in 1:nrow(key_CHIKV_filtered)) {
  temp         <- subset(main_dataset_CHIKV[,c("DATE2", "ISLAND_ID", weather_prec_names)], ISLAND_ID == i)
...
#
###########################################################################
bridgemxgbm <- bridgesampling::bridge_sampler(samples = nuts_fit_1)
