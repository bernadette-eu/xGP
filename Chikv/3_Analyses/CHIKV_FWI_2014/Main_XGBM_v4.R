...
#
###########################################################################
# pc_loc          <- "C://Users//bouranis//"
# main_path       <- paste0(pc_loc, "OneDrive - aueb.gr//BERNADETTE//10_Stan_Project_code//13_Exchange_BM//")
# data_path       <- paste0(main_path, "2_Code//Zika//", "zikachik", ".RData")
# stan_file_path  <- paste0(main_path, "2_Code//Zika//", "hierarchical_TSIR_XGBM_v2", ".stan")
# store_model_out <- paste0(main_path, "2_Code//Zika//", "hierarchical_TSIR_XGBM_v4", ".RData")
# path_to_package <- paste0(pc_loc, "OneDrive - aueb.gr//BERNADETTE//8_Package//bernadette//R//")

# Local machine setup: 
main_path       <- "C://Users//lbour//OneDrive//Desktop1//xgp_project//Zika//"
data_path       <- paste0(main_path, "1_Data//", "zikachik", ".RData")
stan_file_path  <- paste0(main_path, "2_Stan_files//",   "hierarchical_TSIR_XGBM_v2B_chol", ".stan")
store_model_out <- paste0(main_path, "4_Stan_Outputs//", "CHIKV_FWI_2014_XGBM_v3_chol", ".RData")
...
prior_dist_sigma_mu         <-
  prior_mean_sigma_mu       <- prior_scale_sigma_mu      <- prior_df_sigma_mu  <-
  prior_shape_sigma_mu      <- prior_rate_sigma_mu       <-
  prior_dist_sigma_x          <-
  prior_mean_sigma_x        <- prior_scale_sigma_x       <- prior_df_sigma_x   <-
  prior_shape_sigma_x       <- prior_rate_sigma_x        <-
...
prior_params_sigma_mu        <- handle_prior(prior_sigma_mu, ok_dists = ok_dists)
names(prior_params_sigma_mu) <- paste0(names(prior_params_sigma_mu),  "_sigma_mu")

for (i in names(prior_params_sigma_mu)) assign(i, prior_params_sigma_mu[[i]])

prior_params_sigma_x        <- handle_prior(prior_sigma_x, ok_dists = ok_dists)
names(prior_params_sigma_x) <- paste0(names(prior_params_sigma_x),  "_sigma_x")

for (i in names(prior_params_sigma_x)) assign(i, prior_params_sigma_x[[i]])
...
standata <- c(standata_preprocessed,
              nlist(
                ###
                prior_dist_sigma_mu     = prior_dist_sigma_mu,
                prior_mean_sigma_mu     = prior_mean_sigma_mu,
                prior_scale_sigma_mu    = prior_scale_sigma_mu,
                prior_df_sigma_mu       = prior_df_sigma_mu,
                prior_shape_sigma_mu    = prior_shape_sigma_mu,
                prior_rate_sigma_mu     = prior_rate_sigma_mu,
                ###
                prior_dist_sigma_x     = prior_dist_sigma_x,
                prior_mean_sigma_x     = prior_mean_sigma_x,
                prior_scale_sigma_x    = prior_scale_sigma_x,
                prior_df_sigma_x       = prior_df_sigma_x,
                prior_shape_sigma_x    = prior_shape_sigma_x,
                prior_rate_sigma_x     = prior_rate_sigma_x,
                ###
...
#---- Set initial values:
# fit_optim <- rstan::optimizing(m1, data = standata, seed = 1, hessian = TRUE)
# 
# sampler_init <- function(){
#   list(x0       = fit_optim$par[grepl("x0[",      names(fit_optim$par), fixed = TRUE)],
...
#        sigma_mu = fit_optim$par[names(fit_optim$par) %in% "sigma_mu"],
#        sigma_x  = fit_optim$par[names(fit_optim$par) %in% "sigma_x"],
...
#
###########################################################################
bridgeXgbm <- bridgesampling::bridge_sampler(samples = nuts_fit_1)
...
print(bridgeXgbm)
bridgesampling::error_measures(bridgeXgbm)$re2