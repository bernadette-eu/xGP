...
parameters {
	...
	real eta_noise[n_weeks*A];         // Non-daily frequency
...
transformed parameters{  
	...
	matrix<lower = 0>[n_weeks, A] rho_weekly; //<lower = 0, upper = 1>
	...
	for(t in 1:n_weeks) delta_x_mat[t,] = to_row_vector( cholesky_decompose(cov_mat) * eta_noise_mat[t,]');

	//---- Affine transformations for xGBMs:
	// Initiate x_mat at t = 1, assuming that x_{0,j} = 0, j = 1, \ldots, A:
    x_mat[1,:] = delta_x_mat[1,:];
	
	for (j in 1:A) x_mat[2:n_weeks,j] = delta_x_mat[2:n_weeks,j] + x_mat[1:(n_weeks-1),j];
	...
	rho_weekly = inv_logit(x_mat);
    rho_daily  = rep_each_matrix(rho_weekly, gbm_no_days);
	...
	for( t in (N0+1):N ){
		for (j in 1:A) {
			for (k in 1:A) convolution_tmp[k] = contact_matrix[k,j] * dI * dot_product( E_casesAge[1:t-1,k], tail(GT_rev, t-1) );
...
model {
  ...
  // Priors:
  target  += lognormal_lpdf(E_cases_N0 | 4.85, 0.4);    // E_cases_N0 +=// ~ lognormal(4.85, 0.4);
  target  += normal_lpdf(eta_noise | 0, 1);             // eta_noise ~ std_normal();
  target  += normal_lpdf(sigma_x | 0, p_sigma_x);       // sigma_x ~ normal(0, p_sigma_x);
  target  += normal_lpdf(sigma_mu | 0, p_sigma_mu);     // sigma_mu   ~ normal(0, p_sigma_mu)
  target  += normal_lpdf(phiD | 0, p_phiD);             // phiD  ~ normal(0, p_phiD);
