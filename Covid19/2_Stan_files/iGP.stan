...
transformed parameters {
  ...
  logit_p_adjusted[i] = (cholesky_decompose(K_age_group_adjusted[i]) * eta[i]')';
   for (t in 1:T){
         logit_p[i,t] = logit_p_adjusted[i,time_blocks[t]];
...
model{
  for (i in 1:N_age_groups) {
    target += std_normal_lpdf(eta[i, 1:M]);
