...
stan_file_path  <- paste0(main_path, "2_Stan_files//Revision1//", "xGP one country and multiple age groups SE GP - Optimized 1 - with day cp", ".stan")
store_model_out <- paste0(main_path, "4_Stan_Outputs//Revision1//", "xGP", ".RData") # 3D where the ifr$AgrIFR <- c(0.07, 0.021, 0.22, 7.2)/100 are considered
...
sampler_init <- function() {
  # Helper function to extract matrix parameters correctly
  extract_matrix <- function(pname, nrow, ncol) {
    # Use a regex to match the parameter name followed by an open bracket
    # This avoids partial matches (e.g., 'eta' matching 'eta_age_group')
    vals <- fit_optim$par[grepl(paste0("^", pname, "\\["), names(fit_optim$par))]
    return(matrix(vals, nrow = nrow, ncol = ncol))
  }
  
  list(
    # Reshape matrices based on declared dimensions 
    eta_age_group    = extract_matrix("eta_age_group", data_ENG2$N_age_groups,  floor((data_ENG2$T + data_ENG2$gp_days_cp - 1) / data_ENG2$gp_days_cp)),
    cases0           = extract_matrix("cases0",        data_ENG2$N_age_groups, data_ENG2$N0),
    
    # Extract vectors 
    eta_country      = as.vector(fit_optim$par[grepl("^eta_country\\[", names(fit_optim$par))]),
    
    # Extract scalars directly by name
    sigma_age_group  = as.numeric(fit_optim$par["sigma_age_group"]),
    lambda_age_group = as.numeric(fit_optim$par["lambda_age_group"]),
    sigma_country    = as.numeric(fit_optim$par["sigma_country"]),
    lambda_country   = as.numeric(fit_optim$par["lambda_country"]),
    phi              = as.numeric(fit_optim$par["phi"]),
    tau              = as.numeric(fit_optim$par["tau"])
  )
}
