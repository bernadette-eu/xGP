functions {
  ...
  //             K[i, j] = sigma_A[k]^2 * exp(-0.5 * sq_dist_times[i,j] / length_scale_A[k]^2) +
  ...
  matrix compute_covariance_matrix(int M, int[] start_idx, int[] end_idx,
    ...
    int N = rows(sq_dist_times);
    matrix[N, N] K = rep_matrix(0.0, N, N); // final covariance matrix

    real inv_len2_M = 0.5 / square(length_scale_M);

    // Add shared M-level covariance to entire matrix
    for (i in 1:N) {
      for (j in i:N) {
        real v = sigma_M^2 * exp(-inv_len2_M * sq_dist_times[i, j]);
        K[i, j] = v;
    ...
    for (k in 1:M) {
      real inv_len2_A = 0.5 / square(length_scale_A[k]);
      real sigma2_A = square(sigma_A[k]);
      for (i in start_idx[k]:end_idx[k]) {
        for (j in i:end_idx[k]) {
          real v = sigma2_A * exp(-inv_len2_A * sq_dist_times[i, j]);
          K[i, j] += v;
          if (i != j) K[j, i] += v;
        }
      }
